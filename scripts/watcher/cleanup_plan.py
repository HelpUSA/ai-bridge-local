...
def age_minutes(value):
	...
	age = int((now - dt.astimezone(timezone.utc)).total_seconds() // 60)
	return max(0, age)
...
for row in con.execute('select id,command_id,target_chat_id,status,created_at from commands where status=? order by id asc limit ?', ('delivering', args.limit)):
	d = dict(row)
	a = age_minutes(d.get('created_at'))
	d['age_minutes'] = a
	d['stale_candidate'] = bool(a is not None and a >= args.min_age_minutes)
	safe(d)
print('failed_recent_report_only')
for row in con.execute('select id,command_id,target_chat_id,status,created_at from commands where status=? order by id desc limit ?', ('failed', args.limit)):
	safe(dict(row))
con.close()
