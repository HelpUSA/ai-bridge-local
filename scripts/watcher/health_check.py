...
def find_db():
    candidates = [
        ROOT / "queue_local.db",
        ROOT / "runtime" / "queue_local.db",
        ROOT / "data" / "queue_local.db",
        ROOT / "db" / "queue_local.db",
    ]
    ...
    try:
        with sqlite3.connect(db) as con:
            ...
            for table in ["commands", "invalid_messages", "dead_letters"]:
                if table in tables:
                    count = con.execute("select count(1) from " + table).fetchone()[0]
                    print(table + "_count=" + str(count))

            if "commands" in tables:
                rows = con.execute(
                    "select status, count(1) from commands group by status order by status"
                ).fetchall()
                print("commands_status_counts:")
                for status, count in rows:
                    print("- " + str(status) + ": " + str(count))
    except Exception as exc:
        print("database_error=" + repr(exc))
    ...
    for rel in [
        ...
        if rel.endswith(".json"):
            ...
            print(
                rel
                + ": "
                + (" | ".join(found) if found else "version marker not found")
            )

def process_section():
    section("process hints")
    if os.name == "nt":
        run("tasklist python", ["cmd", "/c", "tasklist | findstr /I python"])
        run("tasklist git", ["cmd", "/c", "tasklist | findstr /I git"])
    else:
        run("ps", ["ps", "-ef"])
